        ...
        {enquiries.map((enquiry) => (
          <div key={enquiry._id} className="border rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`px-3 py-1 rounded-full text-sm ${getStatusColor(enquiry.status)}`}>
                {enquiry.status.charAt(0).toUpperCase() + enquiry.status.slice(1)}
              </span>
            </div>
            <p className="text-muted mb-2">{enquiry.message}</p>
            <div className="flex items-center text-sm text-gray-600">
              <Calendar className="w-4 h-4 mr-1" />
              <span>Sent on {new Date(enquiry.createdAt).toLocaleDateString()}</span>
            </div>
          </div>
        ))}
      </div>
